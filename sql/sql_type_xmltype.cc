...
{
  return new (root) Field_xmltype(NULL, (uchar *) "", 1, Field::NONE,
                                 &empty_clex_str, table->s, metadata);
}

bool Type_handler_xmltype::Column_definition_fix_attributes(
...
bool Type_handler_xmltype::Item_hybrid_func_fix_attributes(THD *thd,
...
bool Type_handler_xmltype::Item_sum_sum_fix_length_and_dec(
...
bool Type_handler_xmltype::Item_sum_avg_fix_length_and_dec(
...
bool Type_handler_xmltype::Item_sum_variance_fix_length_and_dec(
       Item_sum_variance *item) const
{
  return Item_func_or_sum_illegal_param(item);
}

bool Type_handler_xmltype::Item_func_round_fix_length_and_dec(
       Item_func_round *item) const
{
  return Item_func_or_sum_illegal_param(item);
}

bool Type_handler_xmltype::Item_func_int_val_fix_length_and_dec(
       Item_func_int_val *item) const
{
  return Item_func_or_sum_illegal_param(item);
}

bool Type_handler_xmltype::Item_func_abs_fix_length_and_dec(Item_func_abs *item) const
{
  return Item_func_or_sum_illegal_param(item);
}

bool Type_handler_xmltype::Item_func_neg_fix_length_and_dec(Item_func_neg *item) const
{
  return Item_func_or_sum_illegal_param(item);
}

bool Type_handler_xmltype::Item_func_signed_fix_length_and_dec(
       Item_func_signed *item) const
{
  return Item_func_or_sum_illegal_param(item);
}

bool Type_handler_xmltype::Item_func_unsigned_fix_length_and_dec(
       Item_func_unsigned *item) const
{
  return Item_func_or_sum_illegal_param(item);
}

bool Type_handler_xmltype::Item_double_typecast_fix_length_and_dec(
       Item_double_typecast *item) const
{
  return Item_func_or_sum_illegal_param(item);
}

bool Type_handler_xmltype::Item_float_typecast_fix_length_and_dec(
       Item_float_typecast *item) const
{
  return Item_func_or_sum_illegal_param(item);
}

bool Type_handler_xmltype::Item_decimal_typecast_fix_length_and_dec(
       Item_decimal_typecast *item) const
{
  return Item_func_or_sum_illegal_param(item);
}

bool Type_handler_xmltype::Item_time_typecast_fix_length_and_dec(
       Item_time_typecast *item) const
{
  return Item_func_or_sum_illegal_param(item);
}

bool Type_handler_xmltype::Item_date_typecast_fix_length_and_dec(
       Item_date_typecast *item) const
{
  return Item_func_or_sum_illegal_param(item);
}

bool Type_handler_xmltype::Item_datetime_typecast_fix_length_and_dec(
...
bool Type_handler_xmltype::Item_char_typecast_fix_length_and_dec(
...
{
  uint from_length_bytes= static_cast<Field_xmltype*>(copy->from_field)->length_bytes;
  uint to_length_bytes=  static_cast<Field_xmltype*>(copy->to_field)->length_bytes;
...
Field::Copy_func *Field_xmltype::get_copy_func(const Field *from) const
{
  if (from->type_handler() != &type_handler_xmltype)
    return do_field_string;
  if (field_length == from->field_length &&
      length_bytes == static_cast<const Field_xmltype*>(from)->length_bytes)
    return do_field_eq;
  return do_copy_vec;
}
...
int Field_xmltype::store(double nr)
{
  return report_wrong_value(ErrConvDouble(nr));
}

int Field_xmltype::store(longlong nr, bool unsigned_val)
{
  return report_wrong_value(ErrConvInteger(Longlong_hybrid(nr, unsigned_val)));
}

int Field_xmltype::store_decimal(const my_decimal *nr)
{
...
int Field_xmltype::store(const char *from, size_t length, CHARSET_INFO *cs)
{
  if (table->in_use->count_cuted_fields != CHECK_FIELD_IGNORE)
  {
    ...
    if (!Type_handler_xmltype::is_valid(from, length))
      return report_wrong_value(ErrConvString(from, length, cs));
  }
...
enum_conv_type Field_xmltype::rpl_conv_type_from(const Conv_source &src,
